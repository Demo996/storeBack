...
if($store) {
    $sql = "SELECT *FROM $store.productor" . $addStr . " LIMIT $limitHead,$pageSize";
    $sqlNum = "SELECT COUNT(*) as number FROM $store.productor" . $addStr;
} else {
    $sql = "SELECT *FROM productor" . $addStr . " LIMIT $limitHead,$pageSize";
    $sqlNum = "SELECT COUNT(*) as number FROM productor" . $addStr;
}
// echo $sql;
