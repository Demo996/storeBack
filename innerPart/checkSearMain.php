...
$searchApplyMan = $handleData["searchApplyMan"];
$limitHead = ($pageNum - 1 ) * $pageSize; // 搜索行数范围之首行

$addStr = "";

if($searchDate || $searchCode || $searchApplyMan) {
    $addStr = " WHERE ";
    if($searchDate) {
        $addStr .= "申请日期='$searchDate' AND ";
    }
    if($searchCode) {
        $addStr .= "申请单编号='$searchCode' AND ";
    }
    if($searchApplyMan) {
        $addStr .= "申报人='$searchApplyMan' AND ";
    }
    $addStr = substr($addStr,0,strlen($addStr)-4);
}

if($store) {
    $sql = "SELECT *FROM $store.already_check_one" . $addStr . " LIMIT $limitHead,$pageSize";
    $sqlNum = "SELECT COUNT(*) as number FROM $store.already_check_one" . $addStr;
} else {
    $sql = "SELECT *FROM already_check_one" . $addStr . " LIMIT $limitHead,$pageSize";
    $sqlNum = "SELECT COUNT(*) as number FROM already_check_one" . $addStr;
}
...
$sendArr["meta"] = $meta;
echo json_encode($sendArr,JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
$conn->close();
?>