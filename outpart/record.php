...
if($store) {
    $sql = "SELECT *FROM $store.receipt" . $addStr . " LIMIT $limitHead,$pageSize";
    $sqlNum = "SELECT COUNT(*) as number FROM $store.receipt" . $addStr;
} else {
    $sql = "SELECT *FROM receipt" . $addStr . " LIMIT $limitHead,$pageSize";
    $sqlNum = "SELECT COUNT(*) as number FROM receipt" . $addStr;
}
